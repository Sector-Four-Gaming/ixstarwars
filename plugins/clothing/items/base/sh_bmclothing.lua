...
ITEM:Hook("drop", function(item)
	...
	local clientap = char:GetData("armorAP")
	local itemap = item:GetData("addap") or 0
	local newap = clientap - itemap
	char:SetData("armorAP", newap)

	PLUGIN:killSpecificClothing(client, item.bmSlot)

	for k, v in pairs(PLUGIN.DefaultClothes) do
		if k == item.bmSlot then
			PLUGIN:createClothing(client, item.bmSlot, v["bmModel"], v["skin"])
		end
	end
end)
			...
			if (char) then
				...
				if (inv) then
					...
					if (attData) then
						...
						local clientap = client:GetData("armorAP") or 0
						client:SetData("armorAP", (clientap - ix.item.list[attData[1]].ap))
...
ITEM.functions.Sell =
{
	...
	OnRun = function(item)
		...
		local clientap = char:GetData("armorAP")
		local itemap = item:GetData("addap")
		local newap = clientap - itemap
		char:SetData("armorAP", newap)
...
function ITEM:OnLoadout()
	if (self:GetData("equip")) then
		...
		timer.Simple( 1, function() 
			...
			local clientap = char:GetData("armorAP")
			local itemap = self:GetData("addap") or 0
			local newap = clientap - itemap
			char:SetData("armorAP", newap)
